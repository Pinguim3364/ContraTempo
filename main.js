...
const MAX_POINTS_CAP = 1e252; 
const REBIRTH_COST = 1e15; // FIXED: 1 Qa (Quadrillion)
...
function formatNumber(num) {
    ...
    const asNumber = isExp ? num.toNumber() : num;
    
    if (isFinite(asNumber) && asNumber < 1000) return asNumber.toFixed(0);
    
    if (isFinite(asNumber) && asNumber < 1e303) {
        let idx = 0;
        let n = asNumber;
        while (n >= 1000 && idx < SUFFIXES.length - 1) { n /= 1000; idx += 1; }
        const decimals = n >= 100 ? 0 : n >= 10 ? 1 : 2;
        return n.toFixed(decimals) + SUFFIXES[idx];
    }
    
    if (isExp) return num.toString();
...
function triggerRebirthCutscene() {
    ...
    const countInterval = setInterval(() => {
        counter++;
        if(pointsEl) pointsEl.textContent = formatNumber(Math.random() * 1e100);
        if (counter > 40) {
...
function checkRebirthAvailable() {
    ...
    const rebirthCostEN = new ExpantaNum(REBIRTH_COST);
    if (state.points.gte(rebirthCostEN)) {
