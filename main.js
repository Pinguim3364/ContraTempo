...
const UPGRADES = [
    {
        ...
        desc: { pt: '+15 P/S por upgrade.', en: '+15 P/S per upgrade.' },
    ...
    },
    {
        ...
        desc: { pt: '2x mais pontos/s.', en: '2x more points/s.' },
    ...
    },
    {
        ...
        desc: { pt: '5x pontos a cada upgrade de Duplicador.', en: '5x points per Duplicator upgrade.' },
    ...
    },
    {
        ...
        desc: { pt: '3.5x mais pontos a cada upgrade de Amplificador.', en: '3.5x more points per Amplifier upgrade.' },
...
function recalcPps() {
    ...
    // Base: +15 P/S per Gerador level
    let baseProd = new ExpantaNum(gen1Lvl).mul(15);
    // Duplicador: 2x per level
    if (gen2Lvl > 0) baseProd = baseProd.mul(new ExpantaNum(2).pow(gen2Lvl));
    // Amplificador: 5x per Duplicador level owned (scales with gen2Lvl, applied once gen3 is bought)
    if (gen3Lvl > 0) baseProd = baseProd.mul(new ExpantaNum(5).pow(Math.min(gen3Lvl, gen2Lvl) || gen3Lvl));
    // Catalisador: 3.5x per Amplificador level owned
    if (gen4Lvl > 0) baseProd = baseProd.mul(new ExpantaNum(3.5).pow(Math.min(gen4Lvl, gen3Lvl) || gen4Lvl));
