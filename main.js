...
const state = {
  ...
  // Rebirth / Prestige
  rebirthMultiplier: 1,       // 1.01^totalUpgradesBought
...
function updateDecayBar() {
  ...
  decayFill.style.width = (pct * 100) + '%';
  decayText.textContent = t('decayPrefix') + state.timeLeft.toFixed(1) + 's';

  if (pct < 0.25) {
    decayFill.classList.add('danger');
  } else {
    decayFill.classList.remove('danger');
  }
}
...
// ---------- JOG AGORA É 100% IDLE - SEM CLIQUE MANUAL ----------

// (handleClick removido - todo progresso vem via P/S automático)

// ---------- QUEDA ----------
function handleDrop() {
  state.stats.totalDrops += 1;

  if (state.survivalTimer > state.stats.recordTime) {
    state.stats.recordTime = state.survivalTimer;
  }
...
function loadGame() {
  try {
    ...
    // Recalcular rebirth multiplier a partir do total de upgrades
    state.stats.totalUpgradesBought = state.stats.totalUpgradesBought || 0;
    state.rebirthMultiplier = Math.pow(1.01, state.stats.totalUpgradesBought);
