...
function criarUpgrades() {
    ...
    for (let branch = 1; branch <= 3; branch++) {
        if (!gameState.upgrades[branch].desbloqueado) {
            ...
            if (branch === 2) {
                textoDesbloqueio = `Desbloqueado ao atingir ${CONFIG.UNLOCK_BRANCH2_PONTOS} pontos`;
            } else if (branch === 3) {
                textoDesbloqueio = `Desbloqueado ao atingir ${CONFIG.UNLOCK_BRANCH3_PONTOS} pontos`;
            }
        ...
        btn.innerHTML = `
            ...
            <div class="upgrade-info">
                <span class="upgrade-gain">+${ganho.toFixed(1)}/s</span>
                <span class="upgrade-cost">Custo: ${custo.toFixed(1)}</span>
...
function atualizarUI() {
    ...
    // Mostrar mini menu após 10 pontos
    if (gameState.pontos >= 10 && !dom.miniMenuBtn.classList.contains('show')) {
        dom.miniMenuBtn.classList.add('show');
    }
}
