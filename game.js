...
(function () {
  ...
  var UPGRADES = [
    ...
    },
    {
      ...
      baseCost: new EN(250000),
      fdcdc: new EN(2.8), // custo escalona a cada rebirth
      isRebirth: true,
      rebirthMult: new EN(3),
      requires: "U4",
      unlocksAt: "U4"
    }
  ];
  ...
  function costOf(u) {
    ...
    if (u.isRebirth) return u.baseCost.mul(u.fdcdc.pow(state.rebirths)); // escalona a cada rebirth
  ...
  function renderCounter() {
    if (tValueEl) tValueEl.textContent = formatT(state.t);
    if (psValueEl) psValueEl.textContent = formatT(basePS());
  ...
  if (themePillSettings) themePillSettings.addEventListener("click", toggleTheme);

  /* ---------------- Toggle de popups (ON/OFF) ---------------- */

  var togglePopupsEl = document.getElementById("toggle-popups");
  function applyPopupsToggle() {
    if (togglePopupsEl) togglePopupsEl.setAttribute("data-on", state.popupsOn ? "true" : "false");
  }
  if (togglePopupsEl) {
  ...
  function applyLang() {
    ...
    for (var i = 0; i < opts.length; i++) {
      var o = opts[i];
      var isActive = o.getAttribute("data-lang") === state.lang;
      if (isActive) o.classList.add("active-lang");
  ...
  if (openSettingsBtn && viewGame && viewSettings) {
  ...
  if (closeSettingsBtn && viewGame && viewSettings) {
  ...
  var confirmDangerBtn = document.getElementById("confirm-danger");

  if (resetSaveBtn && confirmOverlay) {
  ...
  if (confirmDangerBtn) {
    confirmDangerBtn.addEventListener("click", function () {
  ...
  function safe(fn, label) {
    try { fn(); }
    catch (e) { console.error("[t-game] falha em " + label + ":", e); }
  }

  safe(loadGame, "loadGame");
  safe(applyOfflineEarnings, "applyOfflineEarnings");
  safe(applyTheme, "applyTheme");
  safe(applyLang, "applyLang");
  safe(applyPopupsToggle, "applyPopupsToggle");
  safe(function () { if (levelOf("U2") > 0) revealClickButton(); }, "revealClickButton");
  safe(function () { if (levelOf("U3") > 0) startAutoClicker(); }, "startAutoClicker");
  safe(renderUpgrades, "renderUpgrades");
  safe(renderCounter, "renderCounter");
